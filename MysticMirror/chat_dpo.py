...
用法示例：
    python chat_dpo.py
    python chat_dpo.py --model_path ./dpo_output/best_dpo
    python chat_dpo.py --model_path ./dpo_output/best_dpo \
        --device cuda --dtype bf16 --max_new_tokens 512 --temperature 0.8
    python chat_dpo.py --greedy --no_history
"""
import argparse
import os
import re
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="MysticMirror DPO 模型对话脚本",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # ---------- 模型与运行环境 ----------
    parser.add_argument(
        "--model_path", type=str, default="./dpo_output/best_dpo",
...
def load_model(model_path, device, dtype):
    if not os.path.isdir(model_path):
        raise FileNotFoundError(
            ...
            "请先完成 DPO 训练（生成 ./dpo_output/best_dpo），"
