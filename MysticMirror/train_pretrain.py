...
class PretrainDataset(Dataset):
    def __init__(self, texts, tokenizer, max_seq_len: int, allow_empty: bool = False):
        ...
        if len(self.samples) == 0:
            if allow_empty:
                # 评估集允许为空：训练循环中 eval_count==0 会跳过评估与保存
                warnings.warn(
                    "评估集样本为空（评估文本 token 数不足一个分块），将跳过评估。"
                )
            else:
                raise ValueError(
                    "预训练数据集样本为空！总 token 数不足一个 max_seq_len 分块。\n"
                    "请检查：1) 数据文件是否足够大（建议 ≥ 数万行文本）；"
                    "2) max_seq_len 是否过大（可调小，如 512/1024）；"
                    "3) 是否已用 download_data.py 下载 pretrain_t2t_mini.jsonl。"
                )
...
def train_pretrain(
    ...
    if not os.path.isfile(data_path):
        raise FileNotFoundError(
            f"预训练数据文件不存在: {data_path}\n"
            "请先下载语料，例如：\n"
            "  python download_data.py --files pretrain_t2t_mini --limit 50000\n"
            "（或任意自定义 jsonl，每行一个 {\"text\": \"...\"} 对象）"
        )
    ...
    eval_dataset = PretrainDataset(eval_texts, tokenizer, max_seq_len, allow_empty=True)
    if accelerator.is_main_process:
    ...
    # warmup 不超过训练总步数的 5%（小数据下防止 warmup 占满全程导致无衰减）
