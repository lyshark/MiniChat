...
_RESIDUAL_RE = re.compile(r"<think>.*?</think>", re.DOTALL)
...
class AgentChat:
    ...
    def _render_messages(self, messages):
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                # 工具返回：包装为 user 角色 + <tool_response>（与训练一致）
                parts.append(
                    f"{IM_START}user\n{OBS_START}\n{content}\n{OBS_END}{IM_END}\n"
                )
            elif role == "assistant":
    ...
    def _generate_once(self):
        ...
        text = _BUFFER_RE.sub("", text)
        text = _RESIDUAL_RE.sub("", text).strip()
        return text
    ...
    def chat(self, user_input, verbose=True):
        ...
        for _ in range(self.args.max_tool_rounds):
            ...
            if not tool_calls:
                final_answer = content if TOOL_CALL_START in text else text
                if not final_answer.strip():
                    final_answer = "(未能生成有效回复，请重试)"
                ...
                try:
                    ...
                    if verbose:
                        print(f"  [工具调用] {name} {json.dumps(args, ensure_ascii=False)}"
                              f" -> {result}")
                except Exception as exc:  # noqa: BLE001 —— 工具失败也要回填结果
                    result = f"工具执行失败：{exc}"
                    if verbose:
